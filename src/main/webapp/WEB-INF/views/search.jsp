            ...
            <form role="form" method="GET" action="/search">
                <ul class="nav nav-sidebar">
                    ...
                    <li><a><spring:message code="home.type"/></a>
                        <fieldset>
                            <div class="checkbox">
                                <label><input type="checkbox" class="category" value="con"
                                              <c:if test="${type != null && fn:contains(type, 'con')}">checked='checked'</c:if>><spring:message
                                        code="home.con"/></label>
                            </div>
                            <div class="checkbox">
                                <label><input type="checkbox" class="category" value="law"
                                              <c:if test="${type != null && fn:contains(type, 'law')}">checked='checked'</c:if>><spring:message
                                        code="home.law"/></label>
                            </div>
                            <div class="checkbox">
                                <label><input type="checkbox" class="category" value="pd"
                                              <c:if test="${type != null && fn:contains(type, 'pd')}">checked='checked'</c:if>><spring:message
                                        code="home.pd"/></label>
                            </div>
                            <div class="checkbox">
                                <label><input type="checkbox" class="category" value="amc"
                                              <c:if test="${type != null && fn:contains(type, 'amc')}">checked='checked'</c:if>><spring:message
                                        code="home.amc"/></label>
                            </div>
                            <div class="checkbox">
                                <label><input type="checkbox" class="category" value="md"
                                              <c:if test="${type != null && fn:contains(type, 'md')}">checked='checked'</c:if>><spring:message
                                        code="home.md"/></label>
                            </div>
                            <div class="checkbox">
                                <label><input type="checkbox" class="category" value="la"
                                              <c:if test="${type != null && type == 'la'}">checked='checked'</c:if>><spring:message
                                        code="home.la"/></label>
                            </div>
                            <div class="checkbox">
                                <label><input type="checkbox" class="category" value="rp"
                                              <c:if test="${type != null && fn:contains(type, 'rp')}">checked='checked'</c:if>><spring:message
                                        code="home.rp"/></label>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-5">
                                <input class="form-control"
                                       <c:if test="${not empty year}">value="${year}"</c:if> type="number" name="year"
                                       min="1976" max="2015" step="1">
                            </div>
                            <div class="col-md-1" style="font-size: 25px;">/</div>
                            <div class="col-md-5">
                                <input class="form-control" type="text" name="id"
                                       <c:if test="${not empty id}">value="${id}"</c:if> placeholder="N.">
                            </div>
                        </div>
                    </li>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <input class="form-control" type="number"
                                       <c:if test="${not empty fek_year}">value="${fek_year}"</c:if> name="fek_year"
                                       min="1976" max="2015" step="1">
                            </div>
                            <!--<div class="col-md-1" style="font-size: 25px;">/</div>-->
                            <div class="col-md-4">
                                <input class="form-control" type="text" name="fek_id"
                                       <c:if test="${not empty fek_id}">value="${fek_id}"</c:if> placeholder="N.">
                            </div>
                        </div>
                    </li>
                                ...
                                <div class="col-md-6">
                                    <div class='input-group date'>
                                        <input type='text' name="datefrom" id='datepicker2' class="form-control"
                                               <c:if test="${not empty datefrom}">value="${datefrom}"</c:if>
                                               placeholder="<spring:message code="search.from"/>"/>
                                ...
                                <div class="col-md-6">
                                    <div class='input-group date'>
                                        <input type='text' name="dateto" id='datepicker3' class="form-control"
                                               <c:if test="${not empty dateto}">value="${dateto}"</c:if>
                                               placeholder="<spring:message code="search.to"/>"/>
                                        <span class="input-group-addon"><span
                                                class="glyphicon glyphicon-calendar"></span>
                                                    </span>
                                    </div>
            ...
            <div class="table-responsive">
                ...
                <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    ...
                    <c:forEach var="legaldoc" items="${legalDocuments}" varStatus="loop">
                        <tr>
                            <td><a href="${legaldoc.getURI()}"><c:if
                                    test="${not empty legaldoc.getTitle()}">${legaldoc.getTitle()}</c:if><c:if
                                    test="${empty legaldoc.getTitle()}"><spring:message
                                    code="home.${legaldoc.getDecisionType()}"/> ${legaldoc.getYear()}/${legaldoc.getId()} (ΑΓΝΩΣΤΟΣ ΤΙΤΛΟΣ)</c:if></a>
                            </td>
                            <td><spring:message
                                    code="home.${legaldoc.getDecisionType()}"/> ${legaldoc.getYear()}/${legaldoc.getId()}</td>
